...
class DataCommonsAdapter:
    ...
    def fetch(self) -> SourceDataset:
        ...
        raw = response.json()
        values, facet_id, facets = self._parse_observations(raw)
    ...
    def _parse_observations(self, raw: dict[str, Any]) -> tuple[list[SourceValue], str, dict[str, Any]]:
        ...
        for variable_dcid, entry in self.variables.items():
            ...
            available = {str(facet.get("facetId")) for facet in ordered_facets if facet.get("facetId") is not None}
            if not available:
                raise DataCommonsError(f"No facets found for variable {entry}")
            facet_sets.append(available)
            variable_entries.append((variable_dcid, entity_result))

        common_facets = set.intersection(*facet_sets)
        if not common_facets:
            raise DataCommonsError("The five variables do not share a consistent Data Commons facet")
        facet_id = next(iter(common_facets))
        ...
        for category_id, entity_result in variable_entries:
            ...
            values.append(SourceValue(category_id=category_id, category_label=category_id, value=value))
        return values, facet_id, facets
