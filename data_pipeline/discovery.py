...
def _largest_share_limit() -> float:
    try:
...
def _candidate_values(source_values: list[Any], selected_category_ids: list[str] | None) -> list[float]:
...
def _age_start(text: str) -> int | None:
    lowered = text.lower()
...
def _education_start(text: str) -> int | None:
    lowered = text.lower()
    terms = (
...
def _household_size_start(text: str) -> int | None:
    lowered = text.lower()
...
def _ordered_selection(source_values: list[Any]) -> list[str] | None:
    """Select the first four categories for an ordered group, if recognizable."""
    parsers = (_age_start, _education_start, _household_size_start)
